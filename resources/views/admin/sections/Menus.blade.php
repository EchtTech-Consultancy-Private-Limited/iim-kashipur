              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <div>
                     ...
                     <button type="button" class="btn btn-primary" ><a href="{{url('/Accounts/add-edit-sub-child-menu')}}">Add New Sub Child Menu </a></button>
                    </div>
                  </div>
                            ...
                            <tr>
                              ...
                              <td style="display: flex; flex-wrap: wrap; padding-top:0px;" class="td-list">
                                ...
                                <td width="50%" valign="top">
                                    ...
                                    @foreach(@GetsubchildMenus($D->id,$sm->id,$cm->id) as $show)
                                        <p style="margin-right:10px;border-right: 2px solid;padding-right: 5px;">
                                        {{$show->name}}
                                        @if(@checkRoute('Menu_StatusChange'))<a href="{{url('Accounts/menu-status-change/subchildmenu/'.dEncrypt($show->id).'/'.$show->status)}}"> @endif <span  style="background: @if($show->status==0 )red @else green @endif; position: relative; top: -9px; font-size: 11px; color: #fff; padding: 2px;  border-radius: 10px; margin-right:10px;"> @if($show->status==1) Active @else Inactive @endif </span> @if(@checkRoute('Menu_StatusChange'))</a>@endif
                                        <a href="{{url('Accounts/add-edit-sub-child-menu/'.dEncrypt($show->id))}}"><i class="ti-pencil btn-icon-append" style="color:black;"></i></a>
                                        </p>
                                      @endforeach
