                ...
                @endif
                    ...
                    <form role="form" method="POST" action="{{ url('Accounts/manage-quick-links') }}" id="frmtypes">
                        @csrf
                         ...
                         <select  class="form-control"  name="cat_id" class="form-control" onchange="javascript:$('#frmtypes').submit();">
                          <option value="">Filter Section</option>
                            @foreach($value as $k=>$v)
                               <option value="{{$v->id}}" {{ ($v->id==request()->cat_id) ? 'SELECTED' : '' }} >{{$v->Section}}</option>
                            @endforeach
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>{{$D->url}}</td>
                               ...
                               <td><img src="{{ asset('uploads/header_top/'.$D->image) }}" alt="" title="" style="height: 100px;  width: 100px;"></td>


                               <td>
                            @if(@checkRoute('StatusChange'))
                                @if($D->status==1) <a href="{{url('Accounts/status-change/0/'.dEncrypt($D->id).'/quick_links')}}" style="color:green;">Active</a> @else <a href="{{url('Accounts/status-change/1/'.dEncrypt($D->id).'/quick_links')}}" style="color:red;">Inactive</a> @endif
                            @else
