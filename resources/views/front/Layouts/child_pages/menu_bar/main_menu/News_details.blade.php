                    ...
                    @foreach ($item as $items)
                        <div class="col-md-4">
                            <div class="event-card news-card aos-init aos-animate" data-aos="fade-up"
                                ...
                                <a
                                    @if ($items->external == '1') onclick="return confirm('Are you sure  external window open?')"  target="_blank" href="{{ url($items->url) ?? '' }}"  @else href="{{ url($items->url) ?? '' }}" @endif>
                                    <div class="event-body">
                                        <div class="event-image">
                                            <img src="{{ asset('uploads/news_event/' . $items->image) }}"
                                                title="{{ $items->file_title }}" alt="{{ $items->file_alt }}"
