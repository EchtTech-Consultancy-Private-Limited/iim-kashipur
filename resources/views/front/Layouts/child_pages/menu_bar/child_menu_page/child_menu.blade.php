...
@section('content')
    ...
    <div class="internalpagebanner">
        ...
        <div class="imagecaption">
            <div class="container">
                <h1>@if(GetLang()=='en') {{ $type_child[0]->name ?? '' }} @else {{ $type_child[0]->name_h ?? '' }} @endif </h1>
            </div>
        </div>
    </div>
    ...
    <div class="breadcrumbs">
        <div class="container">
            <ul>
                ...
                <li><a href="javascript:void(0);"> @if(GetLang()=='en') {{ $get[0]->name  ?? ''}} @else {{ $get[0]->name_h  ?? ''}} @endif</a></li>
                    <li><span> @if(GetLang()=='en') {{ $gets[0]->name  ?? ''}} @else {{ $gets[0]->name_h  ?? ''}} @endif</span></li>
                    <li><span> @if(GetLang()=='en') {{ $type_child[0]->name  ?? ''}} @else {{ $type_child[0]->name_h  ?? ''}} @endif</span></li>
                    ...
                    <div class="sidebarwraper">
                        ...
                        <ul>
                            ...
                            <li class="hasnested"><a @if($S->id==$gets[0]->id) class="active" @endif> @if(GetLang()=='en') {{ $S->name ?? '' }} @else {{ $S->name_h ?? '' }} @endif<svg class="minus" viewBox="0 0 24 24"><g data-name="Layer 2"><path d="M19 13H5a1 1 0 0 1 0-2h14a1 1 0 0 1 0 2z" data-name="minus"/></g></svg><svg viewBox="0 0 24 24" class="plus"><path d="M19,11H13V5a1,1,0,0,0-2,0v6H5a1,1,0,0,0,0,2h6v6a1,1,0,0,0,2,0V13h6a1,1,0,0,0,0-2Z"/></svg></a>
                                ...
                                <ul>
                                    ...
                                    @foreach(GetchildMenusFront($gets[0]->menu_id,$S->id) as $key2=>$C)
                                        ...
                                        @if($C->external=='yes')
                                            <li><a @if($C->id==$gets[0]->id) class="active" @endif    href="{{url($C->url)}}" onclick="return confirm('Are you sure  external window open?')" target="_blank" > @if(GetLang()=='en') {{ $C->name ?? ''}} @else {{ $C->name_h ?? ''}} @endif</a></li>
                            ...
                            @else
                                ...
                                <li><a href="{{ url($mmenu[0]->slug."/".$S->slug) }}" @if($S->id==$get[0]->id) class="active" @endif>  @if(GetLang()=='en') {{ $S->name ?? ''}} @else {{ $S->name_h ?? ''}} @endif  </a></li>
                               ...
                               <h3><span> @if(GetLang()=='en') {{ $item[0]->name ?? '' }} @else {{ $item[0]->name_h ?? '' }} @endif</span></h3>
                                    @if($item[0]->cover_image!='' && $item[0]->cover_image!='default.jpg')
                                                ...
                                                <div class="col-md-3">
                                                    <div class=" top text-center mt-0">
                                                        <div class="profile-img">
                                                            <img src="{{asset('page/image/'.$item[0]->cover_image ?? '')}}" alt="{{ $item[0]->cover_alt ?? '' }}" title="{{ $item[0]->cover_title ?? '' }}">
                                                        </div>
                                                    </div>
                                                </div>
                                            <div>
                                            ...
                                            <div class="col-md-7 col-lg-8">
                                                <p>  @if(GetLang()=='en') {!! $item[0]->content !!} @else {!! $item[0]->content_h !!} @endif</p>
                                            </div>
                                    @else
